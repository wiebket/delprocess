...
import os
from src.support import usr_dir
...
setup(
      ...
      packages=['dlrprocessing'],
      package_dir = {'dlrprocessing': 'src'},
#      package_data={'dlrprocessing':['data/geometa/site_geo.csv']}, -- not needed if specified in MANIFEST.in
      data_files=[(os.path.join(usr_dir,'specs'), [os.path.join(
                  'src','data','specs', f) for f in [files for root, dirs, files 
                    in os.walk(os.path.join('src','data','specs'))][0]])],
